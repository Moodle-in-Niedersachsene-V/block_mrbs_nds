...
$cfg_mrbs_rlp = get_config('block_mrbs_nds'); //get Moodle config settings for the MRBS block
...
if (!empty($cfg_mrbs_rlp->cronfile) && file_exists($cfg_mrbs_rlp->cronfile)) {
    if ($mrbs_rlp_sessions = fopen($cfg_mrbs_rlp->cronfile, 'r')) {
                ...
                if (($week == 1) and ($date > $now)) {
                    ...
                    if (!import::is_timetabled($csvrow->name, $start_time)) { ////only timetable class if it isn't already timetabled elsewhere (class been moved)
                        ...
                        if ($existingclass = $DB->get_record_sql($sql, [
                            ...
                            $output .= get_string('clash', 'block_mrbs_nds', $a);

                            $existingteacher = $DB->get_record('user', ['id' => $existingclass->create_by]);
                            $newteacher = $DB->get_record('user', ['username' => $csvrow->username]);

                            $body = get_string('clashemailbody', 'block_mrbs_nds', $a);

                            if (email_to_user($existingteacher, $newteacher, get_string('clashemailsub', 'block_mrbs_nds', $a), $body)) {
                                $output .= ', ' . get_string('clashemailsent', 'block_mrbs_nds') . ' ' . $existingteacher->firstname . ' ' . $existingteacher->lastname . '<' . $existingteacher->email . '>';
                            } else {
                                $output .= get_string('clashemailnotsent', 'block_mrbs_nds') . $existingclass->description . '(' . $existingclass->id . ')';
                            }
                            if (email_to_user($newteacher, $existingteacher, get_string('clashemailsub', 'block_mrbs_nds', $a), $body)) {
                                $output .= ', ' . get_string('clashemailsent', 'block_mrbs_nds') . ' ' . $newteacher->firstname . ' ' . $newteacher->lastname . '<' . $newteacher->email . '>';
                            } else {
                                $output .= get_string('clashemailnotsent', 'block_mrbs_nds') . $csvrow->description . '(' . $newentryid . ')';
                            }
                            $output .= "\n";
                        }
                    }
        ...
        if (rename($cfg_mrbs_rlp->cronfile, $cfg_mrbs_rlp->cronfile . '.' . $date)) {
            $output .= $cfg_mrbs_rlp->cronfile . get_string('movedto', 'block_mrbs_nds') . $cfg_mrbs_rlp->cronfile . '.' . $date . "\n";
        }
        $script_time_taken = time() - $script_start_time;
        $output .= get_string('finishedimport', 'block_mrbs_nds', $script_time_taken);
        ...
        if ($mrbs_rlpadmin = $DB->get_record('user', ['email' => $cfg_mrbs_rlp->admin_email])) {
            email_to_user($mrbs_rlpadmin, $mrbs_rlpadmin, get_string('importlog', 'block_mrbs_nds'), $output);
        }
    }
}
...
class import
{
  ...
  //==========================================FUNCTIONS==============================================================
    ...
    {
        $cfg_mrbs_rlp = get_config('block_mrbs_nds');
