...
function mrbs_ndsForceMove($room_id, $starttime, $endtime, $name, $id = null)
{
    global $USER;
    global $DB;
    ...
    foreach ($oldbookings as $oldbooking) {
        ...
        if ($result = $DB->get_record_sql($sizequery, ['5', '50', $shortname])) {
        ...
        $subject = get_string('bookingmoved', 'block_mrbs_nds');
        ...
        if ($findroomresult_check and $DB->update_record('block_mrbs_rlp_entry', $booking) and $oldbookingowner = $DB->get_record('user', ['id' => $oldbooking->create_by])) {
            ...
            if (empty($cfg_mrbs_nds->admin_email)) {
                $output .= '<br>' . get_string('bookingmoveerror', 'block_mrbs_nds', $langvars);
            } else {
                $output .= '<br>' . get_string('bookingmoveerrorshort', 'block_mrbs_nds', $langvars);
            }
            //mail($cfg_mrbs_nds->admin_email, get_string('bookingmoveerror', 'block_mrbs_nds'), get_string('bookingmoveerrormessage', 'block_mrbs_nds', $langvars));
        }
    }
