...
$url = new moodle_url('/blocks/mrbs_nds/web/edit_entry.php', ['id' => $id]);
...
$textarea = $OUTPUT->print_textarea('message', 'message', $message, 15, 350);
//print($textarea);

if (has_capability('block/mrbs_nds:editmrbs', $context) or has_capability('block/mrbs_nds:administermrbs', $context)) {
    echo '<br><br><a href=# onClick="requestVacate.style.visibility=\'visible\';">' . get_string('requestvacate', 'block_mrbs_nds') . '</a>
    ...
    //print_textarea(true, 15, 350, 0, 0, 'message', get_string('requestvacatemessage_html', 'block_mrbs_nds', $messagelang));	
    print($textarea);
	echo '<input type="hidden" name="format" value="' . FORMAT_HTML . '" />';
    //<textarea name = "message" cols=50 rows=10>'.get_string('requestvacatemessage','block_mrbs_nds',$messagelang).'</textarea>
