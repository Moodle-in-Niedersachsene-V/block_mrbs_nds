...
$baseurl = new moodle_url('/blocks/mrbs_nds/web/userweek.php', ['day' => $day, 'month' => $month, 'year' => $year]); // Used as the basis for URLs throughout this file
...
if (!$user || !has_capability('block/mrbs_nds:viewalltt', $context)) {
    $user = $USER->id;
}
$TTUSER = $DB->get_record('user', ['id' => $user]);
...
if ($pview != 1) {
    ...
    $thisweekurl = new moodle_url($baseurl, ['area' => $area, 'room' => $room, 'user' => $user]);
...
for ($j = 0; $j <= ($num_of_days - 1); $j++) {
    ...
    $params = [$TTUSER->id, $TTUSER->username, $pm7[$j], $am7[$j]];
...
for ($t = $starttime; $t <= $endtime; $t += $resolution) {
    ...
    if ($enable_periods) {
        ...
        echo '<a href="' . $hilite_url . '" title="' . get_string('highlight_line', 'block_mrbs_nds') . '">';
    ...
    for ($thisday = 0; $thisday <= ($num_of_days - 1); $thisday++) {
        ...
        if (isset($descrs)) {
            for ($i = 0; $i < count($descrs); $i++) {
                ...
                $viewentry = new moodle_url('/blocks/mrbs_nds/web/view_entry.php', ['id' => $ids[$i], 'area' => $area, 'day' => $wday,
