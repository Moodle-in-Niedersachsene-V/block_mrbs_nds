...
function date_time_string($t)
{
    ...
    return userdate($t, get_string('strftimedaydate', 'langconfig') . ' ' . $timeformat);
}
...
{
    global $twentyfourhour_format;
    $start_date = userdate($starts, get_string('strftimedaydate', 'langconfig'));
    $start_time = userdate($starts, hours_minutes_seconds_format());
    $duration = $ends - $starts;
    if ($start_time == "00:00:00" && $duration == 60 * 60 * 24) {
        return $start_date . " - " . get_string('all_day', 'block_mrbs_nds');
    }
    $dur_units = '';
...
function start_to_end($starts, $ends)
{
    ...
    $end_date = userdate($ends, get_string('strftimedaydate', 'langconfig'));
...
function reporton(&$item, &$last_area_room, &$last_date, $sortby, $display)
{
    ...
    $date = userdate($item->start_time, get_string('strftimedaydate', 'langconfig'));
    // entries to be sorted on area/room
	
	if(is_int($item->create_by)) {
		$userinfos = getUserinfos($item->create_by);
		$erstellt_von = $userinfos->firstname.' '.$userinfos->lastname;
	} else {
		$erstellt_von = $item->create_by;
	}
	
    if ($sortby == "r") {
        if ($area_room != $last_area_room) {
            echo "<hr><h2>" . get_string('room', 'block_mrbs_nds') . ": " . $area_room . "</h2>\n";
        }
        ...
        if ($area_room != $last_area_room || $date != $last_date) {
            echo "<hr noshade=\"true\"><h3>" . get_string('room', 'block_mrbs_nds') . ": " . $area_room . "</h3>\n";
            $last_area_room = $area_room;
        }
    ...
    $viewurl = new moodle_url('/blocks/mrbs_nds/web/view_entry.php', ['id' => $item->id]);
    ...
    echo "<tr><td class=\"BL\" colspan=2><b>" . get_string('type', 'block_mrbs_nds') . "</b> $et</td></tr>\n";
    // Created by and last update timestamp:
    echo "<tr><td class=\"BL\" colspan=2><small><b>" . get_string('createdby', 'block_mrbs_nds') . "</b> " .
    s($erstellt_von) . ", <b>" . get_string('lastmodified') . "</b> " .
...
/*     while (list($name_key) = each($name_hash)) {
    ...
    echo "<hr><h1>" .
    (empty($enable_periods) ? get_string('summary_header', 'block_mrbs_nds') : get_string('summary_header_per', 'block_mrbs_nds')) .
...
$thisurl = new moodle_url('/blocks/mrbs_nds/web/report.php', ['day' => $day, 'month' => $month, 'year' => $year]);
...
<div id="page-content">
  <div id="region-main-box">
    ...
    <h2><?php echo get_string('report_on', 'block_mrbs_nds'); ?></h2>    
    
    <form method="get" action="report.php">
    
      <div id="fitem_id_report_start" class="form-group row fitem">         
          <div class="col-md-3">
              <label class="col-form-label d-inline" for="id_report_start"><?php echo get_string('report_start', 'block_mrbs_nds') ?></label>
          </div>
      ...
      <div id="fitem_id_report_end" class="form-group row fitem">         
          <div class="col-md-3">
              <label class="col-form-label d-inline" for="id_report_end"><?php echo get_string('report_end', 'block_mrbs_nds') ?></label>
          </div>
          <div class="col-md-9 form-inline felement" data-fieldtype="text">
      ...
      <div id="fitem_id_match_area" class="form-group row fitem">         
          <div class="col-md-3">
              <label class="col-form-label d-inline" for="id_match_area"><?php echo get_string('match_area', 'block_mrbs_nds') ?></label>
          </div>
          <div class="col-md-9 form-inline felement" data-fieldtype="text">
      ...
      <div id="fitem_id_roommatch" class="form-group row fitem">         
          <div class="col-md-3">
              <label class="col-form-label d-inline" for="id_roommatch"><?php echo get_string('match_room', 'block_mrbs_nds') ?></label>
          </div>
          <div class="col-md-9 form-inline felement" data-fieldtype="text">
      ...
      <div id="fitem_id_match_type" class="form-group row fitem">         
          <div class="col-md-3">
              <label class="col-form-label d-inline" for="id_match_type"><?php echo get_string('match_type', 'block_mrbs_nds') ?></label>
          </div>
          <div class="col-md-9 form-inline felement" data-fieldtype="text">
              <select class="custom-select" name="typematch[]" multiple="yes">
                ...
                <em><?php echo get_string('ctrl_click_type', 'block_mrbs_nds') ?></em>
          </div>          
      </div>  
      
      <div id="fitem_id_match_entry" class="form-group row fitem">         
          <div class="col-md-3">
              <label class="col-form-label d-inline" for="id_match_entry"><?php echo get_string('match_entry', 'block_mrbs_nds') ?></label>
          </div>
          <div class="col-md-9 form-inline felement" data-fieldtype="text">
      ...
      <div id="fitem_id_match_descr" class="form-group row fitem">         
          <div class="col-md-3">
              <label class="col-form-label d-inline" for="id_match_descr"><?php echo get_string('match_descr', 'block_mrbs_nds') ?></label>
          </div>
          <div class="col-md-9 form-inline felement" data-fieldtype="text">
      ...
      <div id="fitem_id_createdby" class="form-group row fitem">         
          <div class="col-md-3">
              <label class="col-form-label d-inline" for="id_createdby"><?php echo get_string('createdby', 'block_mrbs_nds') ?></label>
          </div>
          <div class="col-md-9 form-inline felement" data-fieldtype="text">
      ...
      <div id="fitem_id_include" class="form-group row fitem">         
          <div class="col-md-3">
              <label class="col-form-label d-inline" for="id_include"><?php echo get_string('include', 'block_mrbs_nds') ?></label>
          </div>
          <div class="col-md-9 form-inline felement" data-fieldtype="text">
                ...
                <?php
                  ...
                  "><label for='summarize1'><?= get_string('report_only', 'block_mrbs_nds'); ?></label>
                </div>                  
                ...
                <div class="form-check form-check-inline">                  
                  ...
                  "><label for='summarize2'><?= get_string('summary_only', 'block_mrbs_nds'); ?></label>
                </div>
                ...
                <div class="form-check form-check-inline">
                  ...
                  "><label for='summarize3'><?= get_string('report_and_summary', 'block_mrbs_nds') ?></label>
                </div>
      ...
      <div id="fitem_id_sort_rep" class="form-group row fitem">         
          <div class="col-md-3">
              <label class="col-form-label d-inline" for="id_sort_rep"><?php echo get_string('sort_rep', 'block_mrbs_nds') ?></label>
          </div>
          <div class="col-md-9 form-inline felement" data-fieldtype="text">
            <div class="form-check form-check-inline">
    ...
    echo "><label for='sortby1'>" . get_string('room', 'block_mrbs_nds'); ?></label>
            </div>
            <div class="form-check form-check-inline">
    ...
    echo "><label for='sortby2'>" . get_string('sort_rep_time', 'block_mrbs_nds'); ?></label>
            </div>
          </div>          
      </div>    
      ...
      <div id="fitem_id_rep_dsp" class="form-group row fitem">         
          <div class="col-md-3">
              <label class="col-form-label d-inline" for="id_rep_dsp"><?php echo get_string('rep_dsp', 'block_mrbs_nds') ?></label>
          </div>
          <div class="col-md-9 form-inline felement" data-fieldtype="text">
            <div class="form-check form-check-inline">
    ...
    echo "><label for='display1'>" . get_string('rep_dsp_dur', 'block_mrbs_nds'); ?></label>
            </div>                
    ...
    echo "><label for='display2'>" . get_string('rep_dsp_end', 'block_mrbs_nds'); ?></label>
            </div>                
          </div>          
      </div>    
      ...
      <div id="fitem_id_summarize_by" class="form-group row fitem">         
          <div class="col-md-3">
              <label class="col-form-label d-inline" for="id_summarize_by"><?php echo get_string('summarize_by', 'block_mrbs_nds') ?></label>
          </div>
          <div class="col-md-9 form-inline felement" data-fieldtype="text">
            <div class="form-check form-check-inline">
    ...
    echo "><label for='sumby1'>" . get_string('sum_by_descrip', 'block_mrbs_nds'); ?></label>
              </div>
    ...
    echo "><label for='sumby2'>" . get_string('sum_by_creator', 'block_mrbs_nds'); ?></label>
              </div>
          </div>          
      </div>                                             

          <em><?= get_string('help_wildcard', 'block_mrbs_nds'); ?></em>
          <br /><hr />
            
          <div id='fitem_id_last_change' class='form-group row fitem'>
            <div class='col-md-3'></div>         
              <div class='col-md-9 felement' data-fieldtype="button"> 
                <button class="btn btn-success" type="submit" name="submitform" value="<?= get_string('submitquery', 'block_mrbs_nds') ?>"><?= get_string('submitquery', 'block_mrbs_nds') ?></button>
            </div>
          </div>  
    </form>
...
if ($submitform) {
    ...
    if ($nmatch == 0) {
        ...
        $last_date = "";
        echo "<P><B>" . $nmatch . " "
        . ($nmatch == 1 ? get_string('entry_found', 'block_mrbs_nds') : get_string('entries_found', 'block_mrbs_nds'))
        . "</B>\n";
