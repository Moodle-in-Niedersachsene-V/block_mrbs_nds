...
<div id="page-content">
  <div id="region-main-box">
    ...
    <h2><?= get_string('advanced_search', 'block_mrbs_nds') ?></h2>
    <form method="get" action="search.php">
    
    <div id="fitem_id_search_str" class="form-group row fitem">
        <div class="col-md-3">
            <label class="col-form-label d-inline" for="id_search_str"><?= get_string('search_for', 'block_mrbs_nds') ?></label>
        </div>
    ...
    <div id="id_search_from" class="form-group row fitem">
        <div class="col-md-3">
            <label class="col-form-label d-inline" for="id_search_from"><?= get_string('from', 'block_mrbs_nds') ?></label>
        </div>
        <div class="col-md-9 form-inline felement" data-fieldtype="text">
...
if (!$search_str) {
    echo "<H3>" . get_string('invalid_search', 'block_mrbs_nds') . "</H3>";
    include "trailer.php";
    exit;
}

// now is used so that we only display entries newer than the current time
echo "<H3>" . get_string('search_results', 'block_mrbs_nds') . " \"<font color=\"blue\">" . s($search_str) . "</font>\"</H3>\n";
...
if (!$total) {
    $total = $DB->count_records_select('block_mrbs_nds_entry', $sql_pred, $params);
...
if ($has_prev || $has_next) {
    echo "<B>" . get_string('records', 'block_mrbs_nds') . ($search_pos + 1) . get_string('through', 'block_mrbs_nds') . ($search_pos + $num_records) . get_string('of', 'block_mrbs_nds') . $total . "</B><BR>";
...
<TABLE BORDER=2 CELLSPACING=0 CELLPADDING=3>
    <TR>
        <TH><?php echo get_string('entry', 'block_mrbs_nds') ?></TH>
        <TH><?php echo get_string('createdby', 'block_mrbs_nds') ?></TH>
        <TH><?php echo get_string('namebooker', 'block_mrbs_nds') ?></TH>
        <TH><?php echo get_string('room', 'block_mrbs_nds') ?></TH>
        <TH><?php echo get_string('description') ?></TH>
        <TH><?php echo get_string('start_date', 'block_mrbs_nds') ?></TH>
    </TR>
    <?php
    foreach ($result as $entry) {
        ...
        $dayurl = new moodle_url('/blocks/mrbs_nds/web/day.php', ['day' => $link['mday'], 'month' => $link['mon'], 'year' => $link['year'],
            'area' => $entry->area_id]);
